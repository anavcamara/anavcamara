<h1 align="center">Hi there, i'm Ana Luiza! </h1>

<p align="center">
  💻 Computer Science student at CESAR School  
  🎨 Front-end Developer | UI/UX Enthusiast  
  🌐 Building digital experiences with purpose
</p>

---
<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=18&pause=1000&color=7F7FD5&center=true&vCenter=true&width=600&lines=Turning+ideas+into+clean+interfaces;Building+real+front-end+projects;Learning+by+creating+and+experimenting" />
</p>

## About Me

Hi! I'm Ana Luiza, a Computer Science student at CESAR School.

During my academic journey, I discovered a strong interest in **web development** and **UI/UX design**, where I can combine logic, creativity, and user experience.

I enjoy turning ideas into functional interfaces, always focusing on **usability, visual clarity, and meaningful user experience**.


---

## 🚀 What I Do

- 💻 Front-end development with HTML, CSS and JavaScript  
- 🎨 UI/UX design and interface prototyping  
- 🧠 Problem solving through code  
- 📚 Continuous learning through academic and personal projects  

---

## 🛠️ Tech Stack

<p align="center">
  <img src="https://img.shields.io/badge/HTML5-0d1117?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-0d1117?style=for-the-badge&logo=css3&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-0d1117?style=for-the-badge&logo=javascript&logoColor=white"/>
  <img src="https://img.shields.io/badge/Python-0d1117?style=for-the-badge&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-0d1117?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/Figma-0d1117?style=for-the-badge&logo=figma&logoColor=white"/>
</p>

---

## 📌 Featured Projects

### • Jornal do Commercio Redesign
A UX/UI redesign project focused on improving the experience of a news platform, enhancing navigation and usability.

### • Hyrox Training System
A Python-based system for managing workouts, exercises, and performance tracking.

### • Personal Portfolio
A website built with HTML, CSS, and JavaScript to showcase my journey, skills, and projects.

### • Formitas
An academic project focused on accessibility and education using Arduino and gamification.

### • Museum of Minds
A narrative game built in Bitsy exploring the history of computing in an interactive way.

---

## Currently Focused On

- Improving front-end development skills  
- Deepening knowledge in UI/UX design  
- Building real-world projects  
- Expanding problem-solving abilities  

---

## 🌐 Connect With Me

- 📧 Email: anacamaradev@gmail.com  
- 💼 LinkedIn: https://www.linkedin.com/in/analuiza-camara  
---

<p align="center">
  <i>“Technology is best when it brings ideas to life.” ✨</i>
</p>
